...
0.9621 PCK@0.05, outperforming the YOLOv8 single-task baselines by
\(+0.273\) mIoU and \(+0.052\) PCK respectively, while unifying both
...
such as YOLOv8 \cite{jocher2023ultralytics} offer unified detection
...
paradigm, while more recent one-stage methods such as YOLOv8-Seg
...
size 16. YOLO baselines use YOLOv8m with the same 256\(\times\)256
...
We compare against two YOLOv8m \cite{jocher2023ultralytics}
variants trained on the same split:
\begin{itemize}
  \item \textbf{YOLO-Seg}: YOLOv8m-seg trained for instance
        segmentation, evaluated on mIoU.
  \item \textbf{YOLO-Pose}: YOLOv8m-pose trained for keypoint
...
\midrule
YOLOv8m-Seg  & Seg only  & 0.5383 & —      \\
YOLOv8m-Pose & Pose only & —      & 0.9105 \\
\midrule
\textbf{HybridMTLNet (Ours)} & \textbf{Seg + Pose} & \textbf{0.8113} & \textbf{0.9621} \\
\midrule
\multicolumn{2}{l}{\(\Delta\) vs.\ YOLO baseline} & +0.273 & +0.052 \\
\bottomrule
\end{tabular}
\end{table}
...
the YOLO-Seg baseline by \(+0.273\) mIoU (\(+50.7\%\) relative)
and the YOLO-Pose baseline by \(+0.052\) PCK (\(+5.7\%\) relative).
...
single-task YOLOv8 baselines on both tasks across a diverse
